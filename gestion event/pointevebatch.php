...
<?php
		...
		foreach($st as $row)
		{
		 echo"<tr><td>$row[intitule]</td><td>$row[description]</td><td>$row[dateDebut]</td><td>$row[dateFin]</td><td>$row[type]</td><td>$row[nom]</td><td><a href='soumibatch.php?eid=$row[eid]'>choisir</td></tr>";
		}
		}
		else
		{	include("header.php");
